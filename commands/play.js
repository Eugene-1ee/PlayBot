	...
	async execute( interaction )
    {
        ...
        if ( val.startsWith( "http" ) )
        {
            if ( val.search("v=") > -1 )
            {
                ...
                {
                    return erremb( interaction, ":triangular_flag_on_post:  **|**  URL이 올바르지 않습니다!", "URL이 올바른지 다시 한번 확인 해주세요!" );
                }
            }
            ...
            {
                return erremb( interaction, ":triangular_flag_on_post:  **|**  URL 형식이 올바르지 않습니다!", "URL이 올바른지 다시 한번 확인 해주세요!" );
            }
        }
        //URL
                ...
                interaction.reply( { embeds: [src] } ).then( async ( ) =>
                {
                    ...
                    }).then( async ( response ) =>
                    {
                        ...
                        if ( response.content == "취소" )
                        {
                            response.delete();
                            interaction.deleteReply();
                            return norpembed( interaction, ":triangular_flag_on_post:  **|**  곡 선택이 취소되었습니다!", "취소를 했다!" );
                        }
                        if ( parseInt(response.content) > 0 && parseInt(response.content) < 11 )
                        {
                            ...
                            {
                                return interaction.channel.send( "재생할 수 없는 영상이야!" );
                            }
                        }
                        else
                        {
                            interaction.deleteReply();
                            return norpembed( interaction, ":triangular_flag_on_post:  **|**  곡 선택이 취소되었습니다!", "올바르지 않은 수가 입력되었습니다. 1 ~ 10 안으로 선택 해주세요." );
                        }
                    } ).catch( (err) =>
                    {
                        return norpembed( interaction, ":triangular_flag_on_post:  **|**  곡 선택이 취소되었습니다!", "곡을 선택하는데 너무 오랜 시간이 걸렸습니다." );
                    } );
                });
