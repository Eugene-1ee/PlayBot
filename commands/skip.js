...
module.exports = {
	...
	async execute( interaction )
    {
        ...
        if (!connection[interaction.guild.id] || getVoiceConnection(interaction.guild.id)._state.status !== "ready") return erremb(interaction, ":triangular_flag_on_post:  **|**  재생 중인 노래가 없습니다!", "스킵 할 노래가 없습니다.");
        if (!playlist[interaction.guild.id]) return erremb(interaction, ":triangular_flag_on_post:  **|**  재생 목록을 찾지 못했습니다!", "이것 참 심오하군요...");
        ...
        {
            return erremb(interaction, ":triangular_flag_on_post:  **|**  " + (val + 1) + "번 트랙을 찾지 못했습니다!", "!재생목록 에서 트랙 번호를 다시 한번 확인 해주세요!");
        }
    }
}